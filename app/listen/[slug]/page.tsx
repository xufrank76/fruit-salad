...
//
// With no ?rendition= param, this is the current, still-open rendition (the
// GET below resolves — and auto-restarts — that same way /record's poll
// does). With one, it's a specific PAST, sealed rendition from the gallery:
// same playback, but no live polling (a sealed rendition never changes) and
// framed as a look back rather than an invitation to keep singing.
function ListenPageInner() {
  const { slug } = useParams<{ slug: string }>();
  const song = getSongBySlug(slug);
  const renditionParam = useSearchParams().get("rendition");
  const viewingPast = !!renditionParam;
  ...
  const renditionUrl = song
    ? renditionParam
      ? `/api/renditions/${song.id}?renditionId=${renditionParam}`
      : `/api/renditions/${song.id}`
    : null;
  ...
  useEffect(() => {
    ...
    fetch(renditionUrl)
      .then((res) => res.json())
      .then((data: { lines: DbLine[]; completedAt: string | null }) => {
        if (cancelled) return;
  ...
  // the record page's poll. Skipped when viewing a specific past rendition —
  // it's sealed, so there's nothing new to pick up.
  useEffect(() => {
    if (viewingPast || !renditionUrl) return;
